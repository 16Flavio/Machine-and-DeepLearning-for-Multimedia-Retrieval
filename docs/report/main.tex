...
\begin{itemize}
    ...
    mise en place d'un moteur \textit{text-to-image} et \textit{image-to-text} sur la base \textit{Flickr8K}, en exploitant le modèle CLIP pour projeter les images et les textes dans un espace latent commun. L'indexation repose sur FAISS afin d'assurer une recherche efficace des plus proches voisins. Un second modèle contrastif (OpenCLIP) est également évalué à titre comparatif.
...
\newpage
\section{Moteur de recherche d'images unimodal à deux niveaux}
\label{sec:part1}
...
\begin{itemize}
    ...
    \emph{locaux} basés points-clés. SIFT produit des descripteurs
    flottants de dimension $128$, ORB des descripteurs binaires de
    $32$~octets~; comme ils ne fournissent pas un vecteur unique de
    taille fixe à l'échelle de l'image, ils sont agrégés via le modèle
    \textit{Bag of Visual Words} (BoVW) détaillé ci-dessous.
\end{itemize}

\paragraph{Le modèle \textit{Bag of Visual Words}.}
Contrairement à HOG, qui produit d'emblée un vecteur de taille fixe,
SIFT et ORB détectent un \emph{nombre variable} de points-clés par
image (de quelques dizaines à plusieurs centaines selon le contenu),
chacun décrit par son propre vecteur local. Une image est donc
représentée par un \emph{ensemble} de descripteurs de cardinalité
variable, que l'on ne peut ni comparer directement par une distance
vectorielle, ni indexer efficacement. Le modèle \textit{Bag of Visual
Words} (BoVW) lève cet obstacle en transposant aux images l'approche
\textit{bag-of-words} de la recherche documentaire~: de même qu'un texte
y est résumé par l'histogramme des occurrences de ses mots sur un
vocabulaire fixe, une image est résumée par l'histogramme de ses
\emph{mots visuels}.

La construction se fait en deux temps. \emph{(i)~Apprentissage du
vocabulaire}~: un large échantillon de descripteurs locaux
($200\,000$ ici) extraits de la galerie est partitionné par
\texttt{MiniBatchKMeans} en $K=256$ groupes~; chaque centroïde définit
un \emph{mot visuel}, c'est-à-dire un motif local prototypique (coin,
jonction de contours, fragment de texture) récurrent dans le corpus.
\emph{(ii)~Encodage}~: pour une image donnée, chacun de ses descripteurs
locaux est assigné au mot visuel le plus proche, et l'on compte les
affectations. On obtient un histogramme $\mathbf{h}\in\mathbb{R}^K$,
normalisé en $L_1$ afin de s'affranchir du nombre total de points-clés
détectés~:
\begin{equation}
    h_w = \frac{1}{|\mathcal{P}|}\sum_{p\,\in\,\mathcal{P}}
          \mathbb{1}\!\left[\,\text{mot}(p) = w\,\right],
    \qquad w = 1,\dots,K,
    \label{eq:bovw}
\end{equation}
où $\mathcal{P}$ est l'ensemble des points-clés de l'image et
$\text{mot}(p)$ le mot visuel assigné au point $p$.

Ce recours à BoVW est \emph{indispensable} pour plusieurs raisons~:
\begin{itemize}
    \item il fournit un descripteur de \textbf{taille fixe} ($K=256$),
    donc directement comparable par les distances vectorielles de la
    Section~\ref{subsec:metrics} et indexable au même titre que les
    autres descripteurs~;
    \item il rend l'encodage \textbf{homogène} entre SIFT (vecteurs
    flottants de dimension $128$) et ORB (vecteurs binaires de
    $32$~octets), tous deux projetés dans un même espace de
    dimension~$K$~;
    \item il \textbf{compacte} fortement la représentation et ramène la
    recherche à un coût \emph{linéaire}, là où le \textit{matching}
    point-à-point direct est quadratique et prohibitif
    ($\sim 35$~s/requête pour SIFT, cf.
    Section~\ref{subsec:results_part1})~;
    \item l'histogramme étant insensible à l'ordre et au nombre des
    points-clés, il confère une certaine robustesse aux occlusions
    partielles et aux variations de point de vue.
\end{itemize}
La contrepartie est la perte de l'\emph{information spatiale}~: le
«~sac~» ignore la position relative des motifs dans l'image, ce qui
limite le pouvoir discriminant de SIFT/ORB-BoVW sur la tâche fine
marque/modèle (cf. Section~\ref{subsec:results_part1}).
...
aux histogrammes BoVW de SIFT et ORB. En revanche, elles ne sont
\emph{pas} adaptées aux embeddings profonds, dont les composantes
peuvent être négatives~: leur application suppose alors de prendre la
valeur absolue des coordonnées, ce qui détruit l'information de signe et
prive le classement de fondement sémantique. Pour les descripteurs
ConvNeXt et DINOv2, seules les distances euclidienne et cosinus sont
donc pertinentes. Le moteur laisse néanmoins les quatre mesures
...
\newpage
%% Marie
\section{Moteur de recherche multimodal}
\label{sec:part2}
...
\begin{figure}[H]
    ...
    \caption{Comparaison de CLIP OpenAI et OpenCLIP LAION sur le dataset}
...
Afin de créer notre interface web, nous avons écrit deux fichiers .html nommés comme suit :
\begin{itemize}
    \item index.html gère la page du moteur de recherche multimodal. 
    \item index2.html gère la page du moteur de recherche d'image unimodal à deux niveaux.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Page d’accueil de la recherche multimodale}
    \label{fig:page_Accueil_index}
\end{figure}

Voici un exemple pour la recherche Texte $\rightarrow$ Images : \newline 

Nous avons testé notre modèle sur deux phrases : "A dog running down the beach" et "Un chien qui court sur la plage". Dans les deux cas, on cherche à retrouver 5 images semblables. \newline
...
Voici un exemple pour la recherche Image $\rightarrow$ Images et Image $\rightarrow$ Texte~: \newline
...
\begin{figure}[H]
    ...
    \caption{Visualisation de l'image requête sélectionnée}
    \label{fig:exemple3.1}
\end{figure}

Nous pouvons voir le résultat de la recherche Image $\rightarrow$ Images sur la Figure \ref{fig:image_to_image}. Pour cette méthode, aucune courbe de rappel et de précision n'est disponible puisque les images téléchargées sont "inconnues" au corpus initial d'entraînement. 
...
Le résultat de la recherche Image $\rightarrow$ Texte est visible sur la Figure \ref{fig:image_to_texts}. Pour cette méthode, aucune courbe de rappel et de précision n'est disponible.
...
\subsubsection*{Moteur de recherche d'image unimodal à deux niveaux}
Lorsque nous passons sur la page de recherche unimodale, nous pouvons observer la page suivante (Figures \ref{fig:unimodalacceuil1} et \ref{fig:unimodalacceuil2}) : 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Autres/unimodal_accueil1.png}
    \caption{Page de recherche d'image unimodal (1)}
    \label{fig:unimodalacceuil1}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Autres/unimodal_accueil_2.png}
    \caption{Page de recherche d'image unimodal (2)}
    \label{fig:unimodalacceuil2}
\end{figure}

Dans cette page, nous pouvons soit rechercher une image de la base de données, soit insérer notre propre image. Ensuite, il est demandé de choisir les descripteurs parmi la liste des 6 descripteurs proposés, sachant qu'on peut sélectionner plusieurs descripteurs, et au maximum tous les descripteurs. Par après, il faut sélectionner une seule métrique. Lorsqu'une métrique est sélectionnée, les autres ne sont plus sélectionnables. Ensuite, on indique le nombre d'éléments que l'on souhaite récupérer. Finalement, on peut choisir si l'on souhaite visualiser les descripteurs (en cliquant sur le bouton \textit{"Visualisation 3D (UMap)"}) ou non. \newline 

Nous avons représenté un exemple ci-dessous (Figures \ref{fig:ex_uni_1} à \ref{fig:ex_uni_5}), où nous avons pris une image de la base de données, avec comme descripteur ORB, associé à la métrique Cosinus, avec un top 14 et la visualisation des descripteurs disponible.
\begin{figure}[H]
    \centering
...
Lors de la visualisation en 3D, nous pouvons choisir un des descripteurs afin de voir sur la carte interactive les différentes classes. \newline
...
et la valeur d'\textit{Average Precision} pour la requête. La sémantique
du champ \texttt{score} dépend du nombre de descripteurs sélectionnés,
et un champ \texttt{score\_type} l'indique explicitement~: pour une
recherche \emph{mono-descripteur}, le score est la \textbf{similarité}
du descripteur ($1{,}0$ pour un appariement parfait)~; pour une
recherche \emph{multi-descripteurs}, c'est le score de fusion
\textit{RRF} (Eq.~\eqref{eq:rrf}), un score de classement borné par
$|\mathcal{F}|/(c+1)$ dont seul l'ordre est interprétable. Le front-end
...
\texttt{python:3.11-slim} pour minimiser la taille~; quatre
dépendances système sont installées via \texttt{apt-get}~: \texttt{git}
(pour récupérer le paquet \textit{CLIP} d'OpenAI, déclaré comme
dépendance Git dans \texttt{pyproject.toml}), \texttt{build-essential}
(compilation des extensions natives lors de l'installation des paquets),
ainsi que \texttt{libgl1} et \texttt{libglib2.0-0} (bibliothèques
requises par les \textit{bindings} OpenCV). Les dépendances Python
...
\end{verbatim}
Une fois les dépendances installées, seul le code source
(\texttt{app.py}, \texttt{src/}, \texttt{Site\_Internet/}) est copié
dans l'image. Les artefacts volumineux (\texttt{models/} et
\texttt{results/}) ainsi que les corpus (\texttt{data/}) ne sont
\emph{délibérément pas} embarqués~: ils sont fournis à l'exécution sous
forme de volumes montés via \texttt{docker-compose.yml}, ce qui maintient
l'image légère et découple le code de ses données. L'image expose le
port \textbf{5000} (\texttt{EXPOSE 5000}) et démarre Flask via la
commande \texttt{python app.py}, qui appelle
...
Le déploiement est orchestré par le fichier \texttt{docker-compose.yml},
qui construit l'image, monte les volumes nécessaires et redémarre
automatiquement le conteneur en cas d'arrêt (\texttt{restart:
unless-stopped}). Une fois Docker installé sur le serveur cible, une
seule commande suffit~:
\begin{verbatim}
docker compose up -d --build
\end{verbatim}
Le service expose le port \textbf{5000} et monte trois volumes~:
\texttt{./data} et \texttt{./models} en lecture seule, \texttt{./results}
en lecture/écriture, auxquels s'ajoute un volume nommé dédié au cache
\textit{Hugging Face}. Cette séparation entre l'image (le code, qui
reste léger) et les données (montées en volumes) évite de reconstruire
l'image à chaque mise à jour des descripteurs et garde l'empreinte du
conteneur réduite. L'application reste ainsi compatible avec un
déploiement sur les services gratuits suggérés dans l'énoncé
(Oracle Free Tier, Render, Railway). Pour un déploiement en production
plus exigeant, il serait préférable d'externaliser les artefacts
volumineux (\textit{checkpoints} \texttt{.pth} et corpus) dans un
\textit{object store} (par exemple S3) plutôt que de dépendre de
volumes locaux.
...
\begin{itemize}
    ...
    \textit{Flickr8K}, ou élargir la comparaison au-delà d'OpenCLIP à
    d'autres familles de modèles vision-langage (BLIP, SigLIP) pour
    quantifier le gain attendu sur ce domaine spécifique~;
